...
const ArticleCard = ({ category, title, author, date, readTime, imageSrc, link, loading }) => (
  <div className={`bg-white rounded-md p-3 shadow-md overflow-hidden ${loading ? 'animate-pulse' : ''}`}>
    <a href={link} className="block">
      {imageSrc ? (
        <img src={imageSrc} className={`w-full h-auto ${loading ? 'bg-gray-300' : ''}`} alt={loading ? 'Loading image' : title} />
      ) : (
        <div className={`w-full h-32 ${loading ? 'bg-gray-300' : ''}`}></div>
      )}
      <div className="p-4">
        <div className={`uppercase text-xs text-gray-500 font-semibold mb-1 ${loading ? 'bg-gray-300 h-4 w-32 rounded' : ''}`}>{category}</div>
        <h3 className={`text-lg font-semibold text-gray-800 line-clamp-2 ${loading ? 'bg-gray-300 h-6 w-48 rounded' : ''}`}>{title}</h3>
        <div className="mt-4 items-center text-gray-600 text-xs ">
          {author ? <span className={loading ? 'bg-gray-300 h-4 w-24 rounded' : ''}>By {author}</span> : <div className={`h-4 w-20 bg-gray-300 rounded ${loading ? '' : 'hidden'}`}></div>}
          <p className={loading ? 'bg-gray-300 h-4 w-40 rounded mt-1' : ''}>
            {date} | <span>{readTime}</span>
          </p>
        </div>
      </div>
    </a>
...
const SidebarLink = ({ label, onClick, loading }) => (
  <button
    onClick={onClick}
    className={`relative flex items-center w-full mt-3 mb-3 px-6 py-3 overflow-hidden font-medium transition-all bg-blue-500 rounded-md group ${loading ? 'animate-pulse bg-gray-300 text-transparent' : ''}`}
    disabled={loading}
  >
    <span
      className={`absolute top-0 right-0 inline-block w-4 h-4 transition-all duration-500 ease-in-out bg-blue-700 rounded group-hover:-mr-4 group-hover:-mt-4 ${loading ? 'bg-gray-400' : ''}`}
    >
      <span
        className={`absolute top-0 right-0 w-5 h-5 rotate-45 translate-x-1/2 -translate-y-1/2 bg-white ${loading ? 'bg-gray-500' : ''}`}
      ></span>
    </span>
    <span
      className={`absolute bottom-0 rotate-180 left-0 inline-block w-4 h-4 transition-all duration-500 ease-in-out bg-blue-700 rounded group-hover:-ml-4 group-hover:-mb-4 ${loading ? 'bg-gray-400' : ''}`}
    >
      <span
        className={`absolute top-0 right-0 w-5 h-5 rotate-45 translate-x-1/2 -translate-y-1/2 bg-white ${loading ? 'bg-gray-500' : ''}`}
      ></span>
    ...
    <span
      className={`absolute bottom-0 left-0 w-full h-full transition-all duration-500 ease-in-out delay-200 -translate-x-full bg-blue-600 rounded-md group-hover:translate-x-0 ${loading ? 'bg-gray-400' : ''}`}
    ></span>
    <span
      className={`relative w-full text-left text-white transition-colors duration-200 ease-in-out group-hover:text-white ${loading ? 'text-transparent' : ''}`}
    >
      {label}
    </span>
  </button>
...
const Blogs = () => {
  ...
  const selectedBlogCards = blogsData.find(blog => blog.blog === selectedTopic)?.card || [];

  return (
    <div className="bg-gradient-to-b from-blue-200 to-white py-8 md:py-10 rounded-md text-center px-4">
      <h2 data-aos="fade-down" data-aos-delay="100" className={`text-2xl md:text-3xl font-semibold text-[#0057D3] mb-4 ${loading ? 'bg-gray-300 h-8 w-96 rounded mx-auto' : ''}`}>
        {loading ? '' : 'Discover Trending Blog Categories'}
      </h2>
      <p data-aos="fade-down" data-aos-delay="200" className={`text-gray-600 text-base mb-6 md:mb-8 ${loading ? 'bg-gray-300 h-6 w-[80%] rounded mx-auto' : ''}`}>
        {loading ? '' : 'Explore the top blog categories, trends, and insights to enhance your knowledge'}
      </p>
      <div data-aos="fade-right" className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {/* Sidebar */}
          <div className="md:col-span-1">
            <div className={`bg-blue-100 w-full p-5 rounded-md shadow-md ${loading ? 'bg-gray-200' : ''}`}>
              <div className={`py-3 px-2 flex items-center justify-between text-gray-800 font-semibold ${loading ? 'text-transparent' : ''}`}>
                Latest Articles
              ...
              <div className="border-t border-gray-200">
                {sidebarLinks.map((link, index) => (
                  <SidebarLink key={index} label={link.label} onClick={link.onClick} loading={loading} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              ...
              selectedBlogCards.map((article, index) => (
                <ArticleCard key={index} {...article} loading={false} />
              ))
            )}
          </div>
        </div>
